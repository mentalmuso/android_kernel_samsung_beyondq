...
int commit_creds(struct cred *new)
{
	...
	if(rkp_cred_enable) {
		...
		if (current->cred == new_ro)
			printk("commit_creds(): Warning, new_ro cred is same with the one of current\n");
...
#endif
{
	...
	if(rkp_cred_enable) {
		...
		if (current->cred == new_ro)
			printk("override_creds(): Warning, new_ro cred is same with the one of current\n");
