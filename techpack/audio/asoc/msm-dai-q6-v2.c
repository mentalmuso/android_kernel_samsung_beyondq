...
static int msm_dai_q6_afe_dynamic_bitpool_get(
            ...
            struct snd_ctl_elem_value *ucontrol)
{
    ...
    ucontrol->value.integer.value[0] = dai_data->dyn_bitpool;
    pr_debug("%s: afe dynamic bitpool : %ld\n",
          __func__, ucontrol->value.integer.value[0]);

    return 0;
}

static int msm_dai_q6_afe_dynamic_bitpool_put(
            struct snd_kcontrol *kcontrol,
            struct snd_ctl_elem_value *ucontrol)
{
    int rc = 0;
    ...
    dai_data->dyn_bitpool = ucontrol->value.integer.value[0];
    pr_debug("%s: updating afe dynamic bitpool : %d\n",
        __func__, dai_data->dyn_bitpool);

    rc = afe_q6_update_dyn_bitpool(dai_data->dyn_bitpool);
    if (rc < 0) {
...
static const struct snd_kcontrol_new quat_mi2s_afe_enc_config_controls[] = {
	...
	SOC_ENUM_EXT("QUAT_MI2S_RX AFE Input Bit Format", afe_bit_format_enum[0],
			msm_dai_q6_afe_input_bit_format_get,
			msm_dai_q6_afe_input_bit_format_put),
	SOC_SINGLE_EXT("QUAT_MI2S_RX AFE Dynamic Bitpool", 0, 0, UINT_MAX, 0,
			msm_dai_q6_afe_dynamic_bitpool_get,
			msm_dai_q6_afe_dynamic_bitpool_put),
