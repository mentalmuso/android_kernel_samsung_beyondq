...
int afe_q6_update_dyn_bitpool(int bitpool)
{
    int ret = 0;
    int index = 0;
    u16 portId =0;
    ...
    memset(&dyn_bitpool, 0, sizeof(dyn_bitpool));
    dyn_bitpool.enc_bitrate = bitpool;
				 ...
				 u32 scrambler_mode)
{
	...
	if (format != ASM_MEDIA_FMT_SBC && format != ASM_MEDIA_FMT_AAC_V2 &&
	    ...
	    format != ASM_MEDIA_FMT_SSC && format != ASM_MEDIA_FMT_APTX_ADAPTIVE &&
