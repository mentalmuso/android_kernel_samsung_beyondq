...
char *auto_comment_buf;
struct debug_reset_header *auto_comment_info;
...
static int sec_auto_comment_info_init(void)
{
	...
	if (auto_comment_info == NULL) {
		pr_err("%s : cannot get auto_comment_info of debug_reset_header!\n", __func__);
		ret = -EINVAL;
		goto error_auto_comment;
	}

	if (auto_comment_info->auto_comment_size == 0) {
		pr_err("%s auto_comment_size [%d], just return!\n", __func__, auto_comment_info->auto_comment_size);
		ret = -ENODATA;
		goto error_auto_comment_info;
	}

	auto_comment_buf = kmalloc(auto_comment_info->auto_comment_size, GFP_KERNEL);
	if (!auto_comment_buf) {
		pr_err("%s : fail - kmalloc for auto_comment_buf\n", __func__);
		ret = -ENOMEM;
		goto error_auto_comment_info;
	}
	...
	if (!read_debug_partition(debug_index_auto_comment, auto_comment_buf)) {
		pr_err("%s : fail - get param!! auto_comment data\n", __func__);
		ret = -ENOENT;
			...
			char __user *buf, size_t len, loff_t *offset)
{
	...
	if (pos >= auto_comment_info->auto_comment_size) {
		pr_info("%s finished: pos %lld, size %d\n", __func__, pos, auto_comment_info->auto_comment_size);
