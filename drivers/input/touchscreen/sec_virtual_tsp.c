...
{
	if (sec->fac_dev->devt == SEC_CLASS_DEVT_TSP1) {
		main_sec = sec;
		input_info(true, sec->fac_dev, "%s: main\n", __func__);
	} else if (sec->fac_dev->devt == SEC_CLASS_DEVT_TSP2) {
		sub_sec = sec;
...
static void sec_virtual_tsp_write_cmd(struct sec_cmd_data *sec, bool main, bool sub)
{
	u8 buff[16];
	int ret = 0;
	bool exit = false;
