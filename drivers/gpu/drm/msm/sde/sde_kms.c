...
static int sde_kms_null_commit(struct drm_device *dev, struct drm_encoder *enc)
{
	struct drm_modeset_acquire_ctx ctx;
	...
	if (!dev || !enc || !enc->crtc) {
		DRM_ERROR("invalid input display\n");
		return -EINVAL;
	}
...
retry:
	...
	if (!conn) {
		SDE_ERROR("error in finding connector for enc:%d", DRMID(enc));
		goto end;
	}
