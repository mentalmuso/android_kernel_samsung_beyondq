...
#include <linux/sec_bsp.h> // to get hw rev, to be removed...
#endif
...
#if defined(CONFIG_SEC_WINNERLTE_PROJECT)
	{
		/* In winner project, it cannot send HS packet to sub panel via DSI1,
		 * in rev00 and rev01 board, due to display connector...
		 * W/A: force to set LP mode for winner sub display.
		 */
		int hw_rev = sec_hw_rev();
		if (vdd->ndx == SECONDARY_DISPLAY_NDX && hw_rev <= 1) {
			state = DSI_CMD_SET_STATE_LP;
			pr_info("[SDE] W/A: set LP for winner DSI1 (%s)\n", ss_get_cmd_name(type));
		}
	}
#endif
