...
static int s2asl01_switching_parse_dt(struct device *dev, struct s2asl01_platform_data *pdata)
{
	...
	if (np == NULL) {
		...
		if(pdata->bat_type & LIMITER_MAIN) {
			...
			ret = pdata->bat_en = of_get_named_gpio_flags(np, "limiter,main_bat_en",
					0, &irq_gpio_flags);
			if (ret < 0) {
				dev_err(dev, "%s : can't main_bat_en\r\n", __FUNCTION__);
			}
			...
			ret = pdata->bat_en = of_get_named_gpio_flags(np, "limiter,sub_bat_en",
					0, &irq_gpio_flags);
			if (ret < 0) {
				dev_err(dev, "%s : can't sub_bat_en\r\n", __FUNCTION__);
			}
		}
				...
				const struct i2c_device_id *id)
{
	...
	pr_info("%s [%s]: int = %d, en = %d\n",
		...
		gpio_get_value(switching->pdata->bat_en));

	//gpio_direction_output(switching->pdata->bat_en, 0);
