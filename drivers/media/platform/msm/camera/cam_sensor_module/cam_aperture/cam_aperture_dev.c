...
ssize_t cam_aperture_test(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	...
	switch (buf[0]) {
	case '0':
		cam_aperture_close(&(g_cam_aperture_t->io_master_info));
		CAM_INFO(CAM_APERTURE, "cam_aperture_close");
		break;
	case '1':
		cam_aperture_open(&(g_cam_aperture_t->io_master_info));
		CAM_INFO(CAM_APERTURE, "cam_aperture_open");
		break;
