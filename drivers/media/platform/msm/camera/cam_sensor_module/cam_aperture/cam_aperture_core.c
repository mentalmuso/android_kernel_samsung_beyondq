...
#if defined(CONFIG_SAMSUNG_OIS_MCU_STM32)
int32_t cam_aperture_set_mode(struct camera_io_master *client, uint32_t power_mode)
{
	int ret = 0;
	...
	if (IRISRUN == 0) {
		...
		ret = cam_aperture_i2c_write(client, IRISMODE, power_mode,
			CAMERA_SENSOR_I2C_TYPE_WORD, CAMERA_SENSOR_I2C_TYPE_BYTE);
		if (ret < 0) {
	...
	void *arg)
{
	...
	case CAM_APERTURE_PACKET_OPCODE_INIT:
		...
		break;
	case CAM_APERTURE_PACKET_OPEN_MODE:
		...
		cam_aperture_open(&a_ctrl->io_master_info); //close aperture mode
		a_ctrl->cam_aper_mode = CAM_APERTURE_1_5;
		break;
	case CAM_APERTURE_PACKET_CLOSE_MODE:
		if (a_ctrl->cam_act_state < CAM_APERTURE_CONFIG) {
			rc = -EINVAL;
			CAM_WARN(CAM_APERTURE,
		...
		cam_aperture_close(&a_ctrl->io_master_info); //open aperture mode
		a_ctrl->cam_aper_mode = CAM_APERTURE_2_4;
		break;
	}
	...
	case CAM_RELEASE_DEV: {
		...
		if (a_ctrl->cam_act_state == CAM_APERTURE_CONFIG) {
			if (a_ctrl->cam_aper_mode == CAM_APERTURE_2_4) {
				...
				cam_aperture_open(&a_ctrl->io_master_info); //1.5
				a_ctrl->cam_aper_mode = CAM_APERTURE_1_5;
			}
...
int cam_aperture_close(struct camera_io_master *client)
{
	int rc = 0;
	...
	//F2.4
	rc = cam_aperture_set_mode(client,IRISCLOSE); ////open aperture mode
	if (rc < 0) {
		CAM_ERR(CAM_APERTURE, "IRISCLOSE fail occurred.");
	}
...
int cam_aperture_open(struct camera_io_master *client)
{
	int rc = 0;
	...
	//F1.5
	rc = cam_aperture_set_mode(client,IRISOPEN); ////close aperture mode
	if (rc < 0) {
