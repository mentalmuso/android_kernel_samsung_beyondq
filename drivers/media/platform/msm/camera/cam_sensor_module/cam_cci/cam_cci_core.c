...
int cci_error_status[2/*device*/][2/*master*/] = { {0, 0}, {0, 0} };
	...
	enum cci_i2c_master_t master)
{
	...
	if (rc < 0) {
...

		{
			...
			uint32_t reg_offset = 0x214;
			uint32_t read_val = cam_io_r_mb(base + reg_offset);
			CAM_INFO(CAM_CCI, "offset = 0x%X value = 0x%X", reg_offset, read_val);
		}
...
static int32_t cam_cci_release(struct v4l2_subdev *sd)
{
	...
	if (rc < 0) {
		...
		CAM_INFO(CAM_CCI, "RELEASE SUCCESFUL");
	}
	...
	struct cam_cci_ctrl *cci_ctrl)
{
	...
	case MSM_CCI_I2C_READ:
		base = cci_dev->soc_info.reg_map[0].mem_base;
		master = cci_ctrl->cci_info->cci_i2c_master;
		reg_offset = DEBUG_MASTER_REG_START + master*0x100 + 0x14;
		read_val = cam_io_r_mb(base + reg_offset);
		if (cci_error_status[cci_dev->soc_info.index][master] == TRUE)
			CAM_INFO(CAM_CCI, "Before read: offset = 0x%X value = 0x%X",
				reg_offset, read_val);
	...
	case MSM_CCI_I2C_WRITE_SYNC_BLOCK:
		...
		read_val = cam_io_r_mb(base + reg_offset);
		if (cci_error_status[cci_dev->soc_info.index][master] == TRUE)
			CAM_INFO(CAM_CCI, "Before write: offset = 0x%X value = 0x%X",
				reg_offset, read_val);
