...
static int32_t cam_sensor_check_tx_off_state(struct camera_io_master *io_master_info)
{
	int tx_off_val = 0;
	int rc = 0;
	struct cam_sensor_i2c_reg_setting  i2c_reg_settings;
	...
	i2c_reg_settings.size = 1;
	i2c_reg_settings.delay = 0;
	i2c_reg_array.reg_addr = 0x602C;
	i2c_reg_array.reg_data = 0x2000;
	i2c_reg_array.delay = 0;
	i2c_reg_array.data_mask = 0x0;
	i2c_reg_settings.reg_setting = &i2c_reg_array;
	...
	i2c_reg_array.reg_addr = 0x602E;
	i2c_reg_array.reg_data = 0xAE08;
	i2c_reg_settings.reg_setting = &i2c_reg_array;

	rc = camera_io_dev_write(io_master_info,
		&i2c_reg_settings);

	camera_io_dev_read(io_master_info, 0x6F12, &tx_off_val,
		CAMERA_SENSOR_I2C_TYPE_WORD, CAMERA_SENSOR_I2C_TYPE_WORD);

	CAM_ERR(CAM_SENSOR, "[SSM_TEST] tx_off : 0x%x", tx_off_val);
	...
	case CAM_SENSOR_PROBE_CMD: {
		...
		if (s_ctrl->soc_info.index == 3) { // check 3P8 or 3P9
			...
			if (rc < 0) {
				...
				if (sensor_id == 0x3109) {
#else
				// Force 3p9 sensor probe success even if no module
				if (sensor_id == 0x3108) {
#endif
					cam_sensor_power_down(s_ctrl);
					goto release_mutex;
...
#if defined(CONFIG_SENSOR_RETENTION)
	if (s_ctrl->sensordata->slave_info.sensor_id == RETENTION_SENSOR_ID) {
		tx_off_val = cam_sensor_check_tx_off_state(&s_ctrl->io_master_info);
		...
		if (sensor_retention_mode == RETENTION_READY_TO_ON) {
			...
			if (tx_off_val == 0xFFF7)
			{
				CAM_INFO(CAM_SENSOR, "To recover tx-off setting in SSM factory test, Set to RETENTION_INIT");
				sensor_retention_mode = RETENTION_INIT;
			}
		}
	} else {
