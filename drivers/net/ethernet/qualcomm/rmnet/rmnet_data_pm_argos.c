...
#define ARGOS_RMNET_TX_AGGR_MBPS 30
		      ...
		      const char *buf, size_t len)
{
	...
	pr_err("rmnet_data_pm_change_rps_map %s %d\n", buf, len);
				  ...
				  unsigned long speed, void *data)
{
	...
	if (speed >= ARGOS_RMNET_PM_QOS_MBPS) {
		if (!pm_qos_requested){
			pr_info("%s pm_qos_request PM_QOS_CPU_DMA_LATENCY PM_QOS_RMNET_LATENCY_VALUE\n", __func__);
		...
		if (pm_qos_requested) {
			pr_info("%s pm_qos_request remove\n", __func__);
			pm_qos_remove_request(&rmnet_qos_req);
			pm_qos_requested = false;
		}
	}

	if (speed >= ARGOS_RMNET_TX_AGGR_MBPS) 
