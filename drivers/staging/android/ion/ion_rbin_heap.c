...
bool is_rbin_heap_enabled(void)
{
	return (rbin_heap != NULL);
}
...
static int rbin_cpu_online(unsigned int cpu)
{
	if (cpumask_any_and(cpu_online_mask, &rbin_cpumask) < nr_cpu_ids) {
		...
		set_cpus_allowed_ptr(rbin_heap->task, &rbin_cpumask);
		set_cpus_allowed_ptr(rbin_heap->task_shrink, &rbin_cpumask);
	}
	return 0;
}
...
#define ALLOC_STR_SIZE	(NUM_ORDERS * (10 + 1))
static int ion_rbin_heap_prereclaim(void *data)
{
	...
	unsigned int nr_pools[NUM_ORDERS];
	unsigned char alloc_str[ALLOC_STR_SIZE];
	unsigned long nr_alloc, nr_pool, nr_free;
	int str_idx, i;
	unsigned long jiffies_s;
	...
	while (true) {
		wait_event_freezable(heap->waitqueue,
				     heap->prereclaim_run);
		memset(nr_pools, 0, sizeof(nr_pools));
		str_idx = 0;
		nr_alloc = atomic_read(&rbin_allocated_pages);
		nr_pool = atomic_read(&rbin_pool_pages);
		nr_free = global_zone_page_state(NR_FREE_RBIN_PAGES);
		totalsize = 0;

		jiffies_s = jiffies;
		task_cputime(current, &utime, &stime_s);
		trace_printk("start\n");
		total_reclaimed = reclaim_contig_migrate_range(heap->base_pfn,
		...
		while (true) {
			...
			nr_pools[order_to_index(order)]++;
		}
		trace_printk("end %lu\n", totalsize);
		for (i = 0; i < NUM_ORDERS; i++)
			str_idx += scnprintf(alloc_str + str_idx,
					    ALLOC_STR_SIZE - str_idx, "%d ",
					    nr_pools[i]);
		alloc_str[str_idx] = '\0';
...
#define K(x) ((x) << (PAGE_SHIFT - 10))
		pr_info("js %u %u apf %lu %lu %lu -> %lu %lu %lu r %lu np %lu %s\n",
			jiffies_to_msecs(jiffies - jiffies_s),
			stime_d / NSEC_PER_MSEC,
			K(nr_alloc), K(nr_pool), K(nr_free),
			K(atomic_read(&rbin_allocated_pages)),
			K(atomic_read(&rbin_pool_pages)),
			K(global_zone_page_state(NR_FREE_RBIN_PAGES)),
			K(total_reclaimed), totalsize >> 10, alloc_str);
...
struct ion_heap *ion_rbin_heap_create(struct ion_platform_heap *data)
{
	...
	heap->heap.type = (enum ion_heap_type)ION_HEAP_TYPE_RBIN;
