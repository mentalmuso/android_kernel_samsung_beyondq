		...
		struct device_attribute *attr, char *buf)
{
	...
	sysfs_scnprintf(buf, info_size, ",\"SoC_REV\":\"%s\"", get_soc_hw_rev());

	sysfs_scnprintf(buf, info_size, ",\"GC_PRM\":\"%d\",\"GC_OPV_3\":\"%d\"",
			get_param0(1), get_param0(2));

	sysfs_scnprintf(buf, info_size, ",\"DOUR\":\"%d\",\"DOUB\":\"%d\"",
			get_param0(7), get_param0(8));
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (p_kinfo->smmu.fsr) {
		...
		if (p_kinfo->smmu.fsynr0) {
			offset += scnprintf((char*)(buf + offset), EXTRA_LEN_STR - offset,
					"\"FSY0\":\"%x\",", p_kinfo->smmu.fsynr0);
			offset += scnprintf((char*)(buf + offset), EXTRA_LEN_STR - offset,
					"\"FSY1\":\"%x\",", p_kinfo->smmu.fsynr1);
			offset += scnprintf((char*)(buf + offset), EXTRA_LEN_STR - offset,
		...
		struct device_attribute *attr, char *buf)
{
	...
	offset += scnprintf((char*)(buf + offset), SPECIAL_LEN_STR - offset,
			",\"HYC\":\"%d\"", p_rst_exinfo->hyp_ex_info.s2_fault_counter);
	offset += scnprintf((char*)(buf + offset), SPECIAL_LEN_STR - offset,
	...
	if (tz_diag_info->magic_num != TZ_DIAG_LOG_MAGIC) {
		...
		if (tz_diag_info->ring_len > 0x200) {	// 512 byte
			...
			offset += scnprintf((char*)(buf + offset), SPECIAL_LEN_STR - offset,
					"\"TZDA\":\"");
			offset += scnprintf((char*)(buf + offset), SPECIAL_LEN_STR - offset,
					"%08X%08X%08X", tz_diag_info->magic_num,
					tz_diag_info->version, tz_diag_info->cpu_count);
			offset += scnprintf((char*)(buf + offset), SPECIAL_LEN_STR - offset,
					"%08X%08X%08X", tz_diag_info->ring_off,
					tz_diag_info->ring_len, tz_diag_info->num_interrupts);
			for (i = 0; i < TZBSP_AES_256_ENCRYPTED_KEY_SIZE; i++) 
				offset += scnprintf((char*)(buf + offset), SPECIAL_LEN_STR - offset,
						"%02X", tz_diag_info->key[i]);
			for (i = 0; i < TZBSP_NONCE_LEN; i++) 
				offset += scnprintf((char*)(buf + offset), SPECIAL_LEN_STR - offset,
						"%02X", tz_diag_info->nonce[i]);
			for (i = 0; i < TZBSP_TAG_LEN; i++) 
				offset += scnprintf((char*)(buf + offset), SPECIAL_LEN_STR - offset,
						"%02X", tz_diag_info->tag[i]);
			offset += scnprintf((char*)(buf + offset), SPECIAL_LEN_STR - offset,
					"%04X%04X", tz_diag_info->ring_buffer.log_pos.wrap, tz_diag_info->ring_buffer.log_pos.offset);
			for (i = 0; i < tz_diag_info->ring_len; i++) 
				offset += scnprintf((char*)(buf + offset), SPECIAL_LEN_STR - offset,
						"%02X", tz_diag_info->ring_buffer.log_buf[i]);
			offset += scnprintf((char*)(buf + offset), SPECIAL_LEN_STR - offset,
					"\"");
		}
	}
