...
#define IPC_LOG_PWR_PAGES	(6*3)
#define IPC_LOG_MISC_PAGES	(10*3)
#define IPC_LOG_TX_RX_PAGES	(8*2)
...
static void wait_for_transfers_inflight(struct uart_port *uport)
{
	...
	if (check_transfers_inflight(uport)) {
		...
		geni_se_dump_dbg_regs(&port->serial_rsc, uport->membase, port->ipc_log_misc);
...
int vote_clock_off(struct uart_port *uport)
{
	...
	IPC_LOG_MSG(port->ipc_log_pwr, "%s%s : Enter \n", 
							...
							unsigned int mctrl)
{
	...
	mb();
	IPC_LOG_MSG(port->ipc_log_misc, 
		"%s: manual_flow=%d, uart_manual_rfr=0x%x \n", 
			__func__, port->manual_flow, uart_manual_rfr);	
}
		...
		if (!pm_runtime_enabled(uport->dev)) {
			if (pm_runtime_status_suspended(uport->dev)) {
				...
				if (ret) {
					...
					if (!port->is_clk_aon) { 
						IPC_LOG_MSG(port->ipc_log_misc, "%s: Enabled IRQ\n", __func__); 
			...
			IPC_LOG_MSG(port->ipc_log_pwr, "%s:ret=%d\n", __func__, ret); 
		}
	}
	return 0;
}
...
static void msm_geni_serial_start_tx(struct uart_port *uport)
{
	...
	return;
check_flow_ctrl:
	IPC_LOG_MSG(msm_port->ipc_log_tx, "%s @check_flow_ctrl --\n", __func__); 
	geni_ios = geni_read_reg_nolog(uport->membase, SE_GENI_IOS);
	//if (!(geni_ios & IO2_DATA_IN))
	IPC_LOG_MSG(msm_port->ipc_log_tx, "%s: ios: 0x%08x\n", 
							__func__, geni_ios);
exit_start_tx:
	if (!uart_console(uport))
		msm_geni_serial_power_off(uport, false);
...
static int msm_geni_serial_sys_suspend(struct device *dev)
{
	...
	if (!uart_console(uport) && port->is_clk_aon && port->startup) {
		IPC_LOG_MSG(port->ipc_log_pwr, "%s: SMP\n", __func__);
		msm_geni_serial_stop_rx(uport);
